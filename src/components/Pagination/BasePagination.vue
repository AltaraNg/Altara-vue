<template>
  <div class="mb-5 w-100 px-0 row align-items-center">
    ...
    <div class="clearfix w-100 mt-4 d-flex bd-highlight">
      <div
        class="col d-flex justify-content-start ml-5 align-items-center pl-0"
      >
        <strong class="light-heading float-left">
          Displaying: {{ pageParam.from }} - {{ pageParam.to }}
          <span v-if="pageParam.total">of {{ pageParam.total }}</span>
        </strong>
      </div>
      ...
      <div class="col d-flex justify-content-center align-items-center">
        ...
        <div class="ml-5">
          ...
          <input
            class="d-inline form-control"
            type="number"
            v-model="pageParam.per_page"
            style="max-width: 50px"
            @keyup.enter="fetchLimit(pageParam.per_page)"
          />
        </div>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      async fetchLimit(page) {
        this.pageParam.per_page = page;

        this.$router.push({
          path: this.$route.path,
          query: {
            ...this.$route.query,
            limit: this.pageParam.per_page,
          },
        });
    ...
    created() {
      ...
      this.pageParam.per_page = this.pageParam.per_page
        ? this.pageParam.per_page
        : 10;
    },
  };
</script>
