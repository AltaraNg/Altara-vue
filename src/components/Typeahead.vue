<template>
  <div
    :class="open ? 'typeahead typeahead__open' : 'typeahead'"
    ...
    computed: {
      filteredOptions() {
        ...
        return this.options.filter(
          (option) => exp.test(option.id) || exp.test(option[this.caption])
        );
      },
    },
    watch: {
      value: function (newVal) {
        if (newVal)
          this.select(this.options.find((option) => option.id === newVal));
      },
    },
    methods: {
      onDownKey() {
        this.filteredOptions.length - 1 > this.selectIndex &&
          this.selectIndex++;
      },
      onUpKey() {
