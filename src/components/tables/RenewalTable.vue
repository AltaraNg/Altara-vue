<template>
    <div>
            ...
            <template v-if="customers.length !== 0">
                <div class="mb-3 row attendance-item mx-auto text-center" :key="index" v-for="(customer, index) in customers">
                    ...
                    >
                        ...
                        <i class="fas fa-exclamation-circle text-success p-1" v-if="calcDebt(customer.amortization) === 0 && mode === 'renewal'"></i>
                    </div>
                    ...
                    >
                        {{ (customer.repayment - calcDebt(customer.amortization)) | currency("₦") }}
                    ...
                    >
                        {{
                            customer.general_feedbacks[0]
                                ? customer.general_feedbacks[customer.general_feedbacks.length - 1].feedback
                                : "N/A" | truncate(25)
                        }}
                    </div>
                    ...
                    <div
                        ...
                        v-if="mode === 'collections'"
                    >
                        <img src="../../assets/css/svgs/circle.svg" alt="plus" @click="addFeedbackModal(customer)" title="Add Feedback" />
                    </div>

                    <div
                        class="col-12 col-xs-3 col-md col-lg d-flex align-items-center justify-content-center"
                        :data-hoverable="customer.last_renewal_prompter_activity ? true : false"
                        @click="viewActivity(customer)"
                        v-if="mode === 'renewal'"
                    >
                        {{
                            customer.last_renewal_prompter_activity
                                ? customer.last_renewal_prompter_activity.attributes.renewal_prompter_status
                                : "" || "N/A"
                        }}
                    </div>
        ...
        <modal name="update-status" :height="'auto'" :clickToClose="false">
            <update-renewal-modal :modalItem="selectedOrderRenew" @close="hide('update-status')" :statuses="statuses" />
        </modal>

        <modal name="add-feedback" :height="'auto'" :clickToClose="false">
            <add-feedback-modal :modalItem="selectedOrderFeedback" @close="hide('add-feedback')" :statuses="statuses" />
        </modal>
        <modal name="general-feedbacks" :height="'auto'" :clickToClose="false">
            <general-feedback-modal :modalItem="selectedOrderGen" @close="hide('general-feedbacks')" />
        </modal>

        <modal name="last-activity" :adaptive="true" :height="'auto'" :clickToClose="false" :reset="true">
            <last-activity-modal :modalItem="selectedActivity" @close="hide('last-activity')" />
        </modal>
    </div>
</template>
...
export default {
    ...
    props: {
        headings: {
            type: Array,
            required: false,
            default: () => ["S/N", "Order Number", "Customer Info Summary", "Repayment Summary", "Last Activity", "Date", "Action"],
        },
        customers: {
            type: Array,
            required: false,
