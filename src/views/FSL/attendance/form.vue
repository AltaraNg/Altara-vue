    ...
    <div class="pt-md-3 pt-2 attendance attendance-create" id="index">
      <div class="mt-5 attendance-head">
        <div class="mb-5 row">
          <div class="col-12 title-con">
            <span class="title">{{
              `attendance for ${today}` | capitalize
            }}</span>
            ...
            >
              <a
                @click="$router.push(`${branch ? '?branch=' + branch : ''}`)"
                class="text-link pr-4 text-capitalize"
                href="javascript:"
              >
      ...
      <div class="mt-5 attendance-head">
        <div class="row px-5 pt-3 pb-4">
          ...
          <div
            class="col-6 col-xs-2 col-md-2 col-lg-2 px-0 px-lg-4 light-heading"
          >
            {{ columns[5] }}
          </div>
          <div class="col-6 col-xs-2 col-md-2 col-lg-2 light-heading">
            {{ columns[6] }}
          </div>
        </div>
      </div>
      ...
      <div class="mt-1 attendance-body">
        <div class="row attendance-item p-5 mb-5" v-if="sentAttendance">
          <span class="no-attendance"
            >You have successfully sent the attendance!</span
          >
        </div>
        <div v-else>
          <form @submit.prevent="onSave" v-if="show">
            <div
              class="mb-3 px-4 row align-items-center attendance-item"
            ...
            >
              ...
              <div class="col-12 col-xs-3 col-md-3 col-lg-3">
                <div class="row">
                  <div class="col pr-3 pr-sm-1 pr-lg-4">
                    <input
                      :disabled="form[index].is_present === '0'"
                      :name="`arrival_time_${index}`"
                      class="form-control"
                      type="time"
                      v-model="form[index].arrival_time"
                      v-validate="form[index].is_present == 1 ? 'required' : ''"
                    />
                  </div>
                  <div class="col pl-3 pl-sm-1 pl-lg-4">
                    <input
                      :disabled="
                        form[index].is_present === '0' || form[index].no_signout
                      "
                      :name="`departure_time_${index}`"
                      class="form-control"
                      type="time"
                      v-model="form[index].departure_time"
                      v-validate="
                        form[index].is_present === '1' &&
                        !form[index].no_signout
                          ? 'required'
                          : ''
                      "
                    />
                  ...
                  <div class="mr-5">
                    <div class="form-check">
                      <input
                        ...
                        type="checkbox"
                        v-model="form[index].no_signout"
                        value="true"
                      />
                      <label
                        class="form-check-label"
                        :for="`remark-${user.user.staff_id}`"
                      ></label>
                    </div>
                  </div>
                </div>
              ...
              <div
                class="col-6 col-xs-2 col-md-2 col-lg-2 pr-3 py-4 py-sm-0 px-sm-0 px-lg-4"
              >
                <div
                  :class="form[index].is_present === '0' ? 'absent' : 'present'"
                  class="clearfix"
                >
              ...
              <div class="col-6 col-xs-2 col-md-2 col-lg-2 pl-3 py-4 py-sm-0">
                <input
                  :disabled="form[index].no_signout"
                  :name="`remark_${index}`"
                  class="form-control"
                  type="text"
          ...
          <div class="row attendance-item p-5 mb-5" v-else>
            <span class="no-attendance"
              >You have already submitted attendance for today!</span
            >
          </div>
        </div>
      </div>
...
<script>
  ...
  const init = ({ branch: b }) =>
    `/api/attendance/create${b ? "?branch=" + b : ""}`;
  ...
  export default {
    ...
    methods: {
      prepareForm(data) {
        if (
          this.$store.getters.auth("peoplesOps") ||
          !this.$route.query["branch"]
        ) {
          this.mode = this.$route.meta.mode;
          if (data.form.length)
            data.form.forEach((obj) => (obj["no_signout"] = false));
          ...
          Flash.setError(
            "You cannot create attendance for a branch other than yours",
            5000
          );
            ...
            if (this.$network()) {
              ...
              post("/api/attendance", { form: this.form })
                ...
                .catch((e) => {
                  e = e.response;
                  if (e.status === 422)
                    Flash.setError(e.data.message ? e.data.message : e.data);
                })
                .finally(() => {
                  this.$LIPS(false);
                  this.$scrollToTop();
      ...
      fetchAttendanceByDate() {
        if (this.newDate) {
          ...
          if (newDate < today) {
            ...
            Flash.setSuccess(
              "You can now proceed and add attendance for " + this.today
            );
          } else
            Flash.setError(
              `Sorry you cannot create attendance for a feature date!`,
              2000
            );
