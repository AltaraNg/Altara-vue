...
class Order {
  ...
  /*custom setters*/
  setIsRepaymentValid() {
    this._isRepaymentValid = !(
      !this.order["repayment_formal"] && !this.order["repayment_informal"]
    );
  }

  setIsOrderFormal() {
    this._isOrderFormal = ["formal", "salaried"].includes(
      this._customer.employment_status.toLowerCase()
    );
  ...
  setRepaymentData() {
    if (this.order["repayment_formal"] != null)
      this._repaymentData = this.order.repayment_formal;
    if (this.order["repayment_informal"] != null)
      this._repaymentData = this.order.repayment_informal;
  }
  ...
  setCountAndInterval() {
    ...
    if (new Date(this.order.order_date) <= new Date("2019-07-07")) {
      ...
      if (
        (Order.isBankDraftAvailable() && this.isOrderFormal) ||
  ...
  setDueDates() {
    this._dueDates = Order.generateDueDates(
  ...
  setCommonDetails() {
    ...
    for (let i = 1; i < this.count + 1; i++) {
      ...
      this._repaymentCaptions.push(
        `<td>${prefix.join("")}<sup>${appendix.join("")}</sup></td>`
      );

      this._actualPayDates.push(
        this.repaymentData[`${vue.$getColumn(i)}_date`]
      );
      this._actualAmountsPaid.push(
        this.repaymentData[`${vue.$getColumn(i)}_pay`]
      );
      this._paymentBanks.push(
        this.repaymentData[`${vue.$getColumn(i)}_payment_bank`]
      );
      this._paymentMethods.push(
        this.repaymentData[`${vue.$getColumn(i)}_payment_method`]
      );
      this._amountsToBePaid.push(
        vue.$roundDownAmt(this.order.repayment_amount)
      );

      if (this.repaymentData[`${vue.$getColumn(i)}_pay`] > 0)
        this._repaymentLevel++;
    }
  }

  setBranch() {
    if (this.order["store_product"]) {
      this._branch = store.state.branches.find(
        (branch) =>
          parseInt(branch.id) ===
          parseInt(this.order.store_product["store_name"])
      );
  ...
  calcAndSetPaymentSummary() {
    ...
    let fmt = (cur) => vue.$formatCurrency(cur),
      { repayment_amount, down_payment, product_price } = this.order;

    /*discount amount*/
    let mFactor = this.count === 6 ? 0.5 : 1,
      discount = this.order?.discount,
      repaymentAsDiscount = discount > 0 ? (discount === 5 ? 1 : 2) : 0,
      discountAmount = vue.$roundDownAmt(
        repayment_amount * mFactor * repaymentAsDiscount
      );
    ...
    if (this.repaymentData) {
      ...
      amountPaid =
        vue.$roundDownAmt(parseInt(down_payment)) +
        totalRepayments +
        discountAmount;
    }
    ...
    /*outstanding debt*/
    this._outstandingDebt = fmt(
      vue.$roundDownAmt(parseInt(product_price) - amountPaid)
    );
  }
  ...
  get customerWGName() {
    let {
      work_guarantor_first_name: a,
      work_guarantor_last_name: b,
      work_guarantor_relationship: c,
    } = this.customer;
    return `${a} ${b} - ${c}`;
  }
  ...
  get customerPGName() {
    let {
      personal_guarantor_first_name: a,
      personal_guarantor_last_name: b,
      personal_guarantor_relationship: c,
    } = this.customer;
    return `${a} ${b} - ${c}`;
  }
