<template>
  <div>
    ...
    <div class="m-auto text-center my-2">
      <button class="bg-default p-2 w-25 mx-2 my-2" @click="searchAction">
        Search
      </button>
      <button class="bg-default p-2 w-25 mx-2 my-2" @click="resetAction">
        Reset
      </button>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      searchAction() {
        this.fromDate !== ""
          ? (this.selectedFilters.fromDate = this.fromDate)
          : "";
        this.toDate !== "" ? (this.selectedFilters.toDate = this.toDate) : "";
        this.status !== "" ? (this.selectedFilters.status = this.status) : "";
        this.orderType !== ""
          ? (this.selectedFilters.orderType = this.orderType)
          : "";
        this.customerName !== ""
          ? (this.selectedFilters.customerName = this.customerName)
          : "";
        this.phoneNumber !== ""
          ? (this.selectedFilters.phoneNumber = this.phoneNumber)
          : "";
