                  ...
                  <tbody>
                      ...
                      <td>
                        {{
                          productItem.created_at
                            ? productItem.created_at.split(" ")[0]
                            : "Not Available"
                        }}
                      </td>
                    </tr>
                    <tr>
                      ...
                      <td>
                        {{
                          productItem.created_at
                            ? productItem.created_at.split(" ")[1]
                            : "Not Available"
                        }}
                      </td>
                    </tr>
  ...
  export default {
    ...
    data() {
      return {
        ...
        response: {},
        show: false,
        headings: ["Name", "Brand", "Category", "Price", "Status"],
        searchColumns: [
    ...
    methods: {
      async fetchData() {
        this.$LIPS(true);
        await get("/api/brand?limit=100")
          .then((res) => {
            Vue.set(this.$data, "brands", res.data.data.data);
          })
          .catch(() => Flash.setError("Error Preparing form"));
        await get("/api/category?limit=100")
          .then((res) => {
      ...
      edit(item) {
        ...
        return this.$router.push({ name: "productEdit", params: { id: item } });
      },
