...
class OrderWithPromiseCall extends Order {
  ...
  setFinancialStatus() {
    this._financialStatus = !this.isRepaymentValid
      ? "no detail!"
      : `Paid: ${this.amountPaid} | Debt: ${this.outstandingDebt}`;
  }
  ...
  generateAndSetNextSMSReminder() {
    ...
    if (order_date === vue.$getDate()) {
      message =
        `Hello ${this.customerName}, thanks for patronizing us.` +
        ` The following is the breakdown of the repayment plan for` +
        ` the purchase of ${product_name}:%0a`;
      this.dueDates.forEach(
        (date, index) =>
          (message +=
            vue.$getColumn(index + 1) +
            ": " +
            date +
            " => " +
            vue.$formatCurrency(vue.$roundDownAmt(repayment_amount)) +
            "%0a")
      );
    } else {
      message =
        `Hello ${this.customerName}, This is to remind you that your` +
        ` ${vue.$getColumn(parseInt(this.repaymentLevel) + 1)} repayment of` +
        ` ${vue.$formatCurrency(
          vue.$roundDownAmt(repayment_amount)
        )} for ${product_name}` +
    ...
    this._nextSMSReminder =
      message +
      "Please remember not to make payment in any form through any Altara Agent" +
