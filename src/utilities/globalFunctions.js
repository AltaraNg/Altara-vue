...
Vue.prototype.$prepareUncontacted = () => {
  !store.getters.getUncontacted &&
    get(
      "api/renewal/prompters?filterOrderbyBranch=true&orderHasAtMostTwoPaymentsLeft=true&unContactedRenewalPrompters=true&tab=nc"
    ).then((r) =>
      store.dispatch("mutateUncontacted", r.data.data.renewal_prompters.total)
    );
};
...
Vue.prototype.$getCustomerOfficeAddress = (c) =>
  c
    ? `${c.comp_house_no} ${c.comp_street_name} ${c.comp_area}, ${c.company_city}, ${c.company_state}.`
    : null;
...
Vue.prototype.$prepareBranches = () => {
  !store.getters.getBranches &&
    get("/api/branches").then((r) =>
      store.dispatch("mutateBranches", r.data.branches)
    );
};
...
Vue.prototype.$prepareStates = () => {
  !store.getters.getStates &&
    get("/api/state").then((r) =>
      store.dispatch("mutateStates", r.data.states)
    );
};

Vue.prototype.$preparePaymentMethods = () => {
  !store.getters.getPaymentMethods &&
    get("/api/payment_method").then((r) =>
      store.dispatch("mutatePaymentMethods", r.data.paymentMethods)
    );
};

Vue.prototype.$prepareBanks = () => {
  !store.getters.getBanks &&
    get("/api/bank").then((r) => store.dispatch("mutateBanks", r.data.banks));
};

Vue.prototype.$prepareTypeaheadUsersList = () => {
  !store.getters.getTypeaheadUsersList &&
    get("/api/users/list_type/type_ahead").then((r) =>
      store.dispatch("mutateTypeaheadUsersList", r.data.users)
    );
};
    ...
    if (store.getters.auth("DSAAccess")) {
      ...
      if (store.state.DSALead.includes(user.role_id)) {
        ...
        if (store.state.DSACaptain.includes(user.role_id)) {
          ...
          if (user.id === customer.user.id) {
            return true;
          } /**else if its not the same person deny access*/ else return true;
        }
      }
      ...
      if (store.state.DVALead.includes(user.role_id)) {
        ...
        if (user.branch_id === customer.branch_id) {
          ...
          return true;
        } /**else is both branch doesnt match deny access*/ else return true;
      }
    } /**if user is not dva or dsa: no access ie false*/ else return false;
  } /**if there is no user or customer deny access*/ else return false;
};
