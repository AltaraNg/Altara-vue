<template>
  <div class="my-3 py-4" v-if="reports !== null">
    ...
    <div class="row px-4 h-auto" v-if="reports !== null">
      ...
      >
        <template v-slot:svg
          ><img src="../../assets/new_stats.png" alt="work" style="height=100%"
        /></template>
      </stat-card>
      <stat-card
      ...
      >
        <template v-slot:svg
          ><img src="../../assets/stat_red.png" alt="work" style="height=100%"
        /></template>
      </stat-card>
      ...
      <div class="card col p-5" v-if="reports !== null">
        ...
        <div class="text-left">
          ...
          <ul class=".list-unstyled">
            <li>
              <span class="font-weight-bold" style="color: #e2e612"
                >Active Sales</span
              >:
              <span
                >anyone who has not completed payment but has made a payment in
                the last 60 days(2 months) (from today's date), REGARDLESS of if
                the payment cycle is over.</span
              >
            </li>
            <li>
              <span class="font-weight-bold" style="color: #e6091c"
                >Inactive Sales</span
              >:
              <span>
                anyone who has not completed payment but has not made any
                payment in the last 60 days</span
              >
            </li>
            <li>
              <span class="font-weight-bold" style="color: #2ad413"
                >Completed Sales</span
              >:
              <span
                >anyone who has completed payment irregardless of if they
                completed it on time or late.
              </span>
            </li>
            <li v-if="checker">
              <span class="font-weight-bold" style="color: #aaaaaa"
                >No Orders </span
              >: <span>No orders for that time frame </span>
            </li>
          </ul>
        </div>
      </div>
    ...
    methods: {
      getPieChartData() {
        this.pieData = {
          labels:
            this.checker === true
              ? ["No orders"]
              : ["Active Orders", "Inactive Orders", "Completed Orders"],
          options: {
            title: {
              text: [
                "Order Categories",
                "Chart describing number of active",
                "inactive and completed orders per selected period of time",
              ],
              display: true,
              color: ["#ffffff"],
            },
          ...
          datasets: [
            {
              ...
              backgroundColor: this.checker
                ? ["#aaaaaa"]
                : ["#e2e612", "#e6091c", "#2ad413"],
            },
          ],
        };
      },
        ...
        this.polarData = {
          ...
          options: {
            title: {
              text: [
                "Overdue Days",
                "Chart of how many days overdue for missed payments",
              ],
              display: true,
              color: "#ffffff",
            },
      ...
      getPieData() {
        ...
        return this.checker === true
          ? [1]
          : [orderStatus.active, orderStatus.inactive, orderStatus.complete];
      },
