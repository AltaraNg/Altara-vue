        ...
        <form @submit.prevent="onSave">
          <div class="my-4 clearfix p-5 row bg-white shadow-sm card-radius">
            <div class="form-group col-12 float-left px-0 px-md-3">
              ...
              <small v-if="errors.first('brand name')">{{
                errors.first("brand name")
              }}</small>
  ...
  export default {
    ...
    methods: {
      ...
      addFinish() {
        ...
        patch(`/api/brand/${this.$route.params.id}/categories`, data)
          .then((res) => {
            this.$swal({
              icon: "success",
              title: res.message,
              ...
              (this.mode === "edit"
                ...
                .then(({ data }) => {
                  if (data.status === "success") {
                    ...
                    return this.$router.push({ path: "/log/brands" });
                  }
                })
                .catch(({ response: r }) => {
                  let { data, status } = r;
