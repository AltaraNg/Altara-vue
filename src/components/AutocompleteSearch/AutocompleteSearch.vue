<template>
  <transition name="fade">
    <div
      class="pt-md-3 pt-2 attendance-view mb-4 z-2 position-relative"
      id="index"
    >
      ...
      <div class="attendance-body">
        <div class="p-5 row bg-white shadow-sm card-radius">
          <div class="col px-0 px-md-3">
            ...
            <div
              class="col-md-6 col-12 float-left d-flex justify-content-around align-items-center"
            >
              <span
                class="position-relative radio mb-0"
    ...
    methods: {
      ...
      getAutocomplete(option) {
        ...
        const fullName = option.hasOwnProperty("full_name")
          ? option.full_name
          : this.$getCustomerFullName(option, !!option.middle_name);
      ...
      styleAutocomplete(optionArr) {
        let HTMLElement = "";
        optionArr.forEach(
          (option) => (HTMLElement += `<span class="col">${option}</span>`)
        );
        return HTMLElement;
      },
      ...
      isStringLengthValid(str) {
        ...
        return (
          (this.searchColumn === "name" && wordCount <= 3) || wordCount <= 1
        );
      },
    },
  };
