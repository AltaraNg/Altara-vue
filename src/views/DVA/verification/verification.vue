<template>
    <transition name="fade">
        <div class="pt-md-3 pt-2 verification overflow-hidden" id="employeeRegister">
            <AutocompleteSearch title="customer verification" @customer-selected="processForm" :url="'/api/customer/autocomplete'" />
                ...
                <div v-if="customer">
                    ...
                    <div style="display: flex" v-if="action !== 'update'">
                        ...
                        <custom-header
                            ...
                            :style="!states.verification ? 'opacity: 0.2; cursor:pointer;' : 'cursor:pointer; '"
                        ...
                        <custom-header
                            ...
                            :style="!states.credit_report ? 'opacity: 0.2; cursor:pointer;' : 'cursor:pointer; '"
                            style="margin-left: -10px"
                        />
              ...
              <div class="title">
                ...
                Add More
                    ...
                    <div v-if="action !== 'update'" class="w-100">
                        <template v-if="states.order">
                            <div class="float-left col-lg-3 col-sm-6 px-0 px-sm-3" v-for="type in cardView" :key="type">
                        ...
                        <template v-if="states.order">
                            <div class="float-left col-lg-3 col-sm-6 px-0 px-sm-3" v-for="type in newDocView" :key="type">
                                <div :class="DivClassNewDoc(type)" class="card card-stats">
                                    ...
                                    <div @click="modal(type + '_modal')" class="card-footer pointer d-flex flex-wrap justify-content-between">
                                        <div>
                                            <i class="now-ui-icons ui-1_calendar-60 pr-1"></i>
                                            {{ displayActiveDocument(type) ? "Verified" : "Not Verified" }}
                                            <small v-if="!displayActiveDocument(type)" style="font-size: 9px">(Click here to update status!)</small>
                                        </div>
                                        <div v-for="(item, index) in customer.new_documents" :key="index" class="">
                                            <div v-if="item.name === type">
                                                <span class="float-right" style="font-size: 10px"> by - {{ item.user | capitalize }} </span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div v-if="states.verification" class="mb-5">
                            <div class="mt-5 mb-3 attendance-head">
                                <div class="row px-4 pt-3 pb-4 text-center">
                                    <div class="col light-heading" style="max-width: 150px">S/No.</div>
                                    <div
                            ...
                            <div class="tab-content mt-1 attendance-body">
                                <div class="tab-pane active" v-if="verificationList.length > 0">
                                    <div class="mb-3 row attendance-item text-center" v-for="(verification, index) in verificationList" :key="index">
                                        <div class="col-12 col-xs-2 col-md col-lg d-flex" style="max-width: 150px; margin-left: -15px">
                                            <span class="user mx-auto">{{ index + 1 }}</span>
                                        </div>
                                        <div class="col-12 col-xs-2 col-md col-lg d-flex user-name align-items-center" style="padding-left: 30px">
                                        ...
                                        <div class="col-12 col-xs-2 col-md col-lg d-flex align-items-center">
                                            <button @click="displayActiveVerification(verification)" class="btn status approved">View</button>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div v-if="states.credit_report" class="mb-5">
                            ...
                            <div class="tab-content mt-1 attendance-body">
                                <div class="tab-pane active" v-if="creditReportList.length > 0">
                                    <div class="mb-3 row attendance-item text-center" v-for="(creditReport, index) in creditReportList" :key="index">
                                        <div class="col-12 col-xs-2 col-md col-lg d-flex" style="max-width: 150px; margin-left: -15px">
                                            <span
                                        ...
                                        <div class="col-12 col-xs-2 col-md col-lg d-flex align-items-center">
                                            <button @click="displayActiveCreditReport(creditReport)" class="btn status approved">View</button>
                                        </div>
                                    </div>
                                </div>
            ...
            <div v-if="action !== 'update'">
                <div :id="type + '_modal'" class="modal fade" v-for="type in picsView" :key="type">
                ...
                <div :id="type + '_modal'" class="modal fade" v-for="(type, index) in newDocView" :key="index">
                    <div class="modal-dialog" :style="{ pointerEvents: type == 'passport' || type == 'id_card' ? 'none' : 'auto' }">
                        <div class="modal-content">
                            ...
                            <form @submit.prevent="saveNewDoc(type, type + '_modal')" enctype="multipart/form-data" v-if="customer">
                                <div class="modal-body">
                                    <div class="bg-default text-white">max-size: 512kb max-dimension: 1200 x 1200</div>
                                    <div class="upload-image p-2">
                                    ...
                                    <div class="w-100 p-3">
                                        ...
                                        <div class="form-group clearfix">
                                            ...
                                            <span class="radio ml-5">
                                                <input id="no" name="customer_meetup" type="radio" v-model="address.customer_meetup" value="no" />
                                        ...
                                        <div class="form-group clearfix">
                                            <label> 2. Is the address/location same with what you have in the application form? </label>
                                            <span class="radio mx-5">
                                                <input
                                            ...
                                            <span class="radio ml-5">
                                                <input id="add_no" name="confirm_address" type="radio" v-model="address.confirm_address" value="no" />
                                        ...
                                        <div class="form-group clearfix">
                                            <label
                                                >4. Look around the shop and check the nature and condition of the business. Write down what you see
                                                in terms of address, stock value, premise, type of shop or business, sales etc.</label
                                            >
                                            <textarea
                                                class="form-control"
                                        ...
                                        <div class="form-group clearfix">
                                            <label
                                                >5. Get exact information of choice of product and specification by asking for Example; what exact
                                                phone do you want? Let him/her specify e.g. INFINIX SMART or HOT 4 IPHONE
                                            </label>
                                            <textarea
                                        ...
                                        <div class="form-group clearfix">
                                            <label> 6. Are you aware of the payment plan? </label>
                                            <span class="radio mx-5">
                                                <input
                                            ...
                                            <span class="radio ml-5">
                                                <input id="pay_no" name="aware_of_plan" type="radio" v-model="address.aware_of_plan" value="no" />
                                        ...
                                        <div class="form-group clearfix">
                                            <label> 7. Did you get information about the customer from his/her neighbours? </label>
                                            <span class="radio mx-5">
                                                <input
                                        ...
                                        <div class="form-group clearfix">
                                            <label>8. And how long has he/she been working/trading in that particular place? </label>
                                            <textarea
                                                class="form-control"
                                        ...
                                        <h5 v-else>This customers details has already been updated once and cannot be changed!</h5>
                                    </div>
                                </div>
                            </form>
                        </div>
                ...
                <div :id="type + '_modal'" class="modal fade" v-for="type in veriView" :key="type">
                                    ...
                                    <div class="form-group col-12 px-2 clearfix my-0">
                                        ...
                                        <div class="clearfix">
                                            ...
                                            <div v-if="type !== 'processing_fee'">
                                                ...
                                                <div class="radio p-0 col-6 float-left">
                                                    <input :id="type + '_no'" name="consent" type="radio" v-model="$data[type].consent" value="0" />
                    ...
                    if (result) {
                        await post(`/api/${type}`, this[type])
                            .then(({ data }) => {
                                ...
                                if (type === "address") action += this.address.approval_status ? "Passed" : "NotPassed";
                                log(action, id);
