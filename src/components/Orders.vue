<template>
  <div>
              ...
              <table class="table table-bordered table-striped">
                <tbody>
                  ...
                  <tr>
                    <th>Repayment</th>
                    <td>
                      {{ $formatCurrency(activeOrder.amountsToBePaid[0]) }}
                    </td>
                  </tr>
                  <tr>
                    <th>Down Payment</th>
                    <td>
                      {{ $formatCurrency(activeOrder.order.down_payment) }}
                    </td>
                  </tr>
                  <tr>
                    <th>Discount (%)</th>
                  ...
                  <tr>
                    <th>Sale Type</th>
                    <td>
                      {{ activeOrder.order.sales_type.name | capitalize }}
                    </td>
                  </tr>
                  <tr>
                    <th>Total amount to Pay</th>
                    <td>
                      {{ $formatCurrency(activeOrder.order.product_price) }}
                    </td>
                  </tr>
                  <tr>
                    <th>Processed by</th>
                    <td>
                      {{
                        activeOrder.order["floor_agent"]
                          ? activeOrder.order.floor_agent.full_name
                          : null
                      }}
                    </td>
                  </tr>
    ...
    <div class="modal fade" id="customer_info">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
              ...
              <table class="table table-bordered table-striped">
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>{{ activeOrder.customerWGName }}</td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    ...
                    <td>{{ activeOrder.customerPGName }}</td>
                  </tr>
                  <tr>
          ...
          <div class="modal-footer">
            <a
              class="text-link mt-3 w-100 text-right"
              data-dismiss="modal"
              href="#"
            >
              close dialogue
        ...
        <div class="modal-content" v-if="showModalContent">
          <div class="modal-header py-2">
            <h6 class="modal-title py-1">
              ...
              {{ activeOrder.customer.employment_status | capitalize }}
            </h6>
            <a aria-label="Close" class="close py-1" data-dismiss="modal">
              <span aria-hidden="true" class="modal-close text-danger">
                <i class="fas fa-times"></i>
              ...
              <div class="amor-table">
                <table class="table table-bordered">
                  <tbody class="text-center">
                    ...
                    <tr class="table-separator">
                      <th>Due Date</th>
                      <td v-for="date in activeOrder.dueDates" :key="date">
                        {{ date }}
                      </td>
                    </tr>
                    <tr>
                      ...
                      <td
                        v-for="date in activeOrder.actualPayDates"
                        :key="date"
                      >
                        {{ date }}
                      </td>
          ...
          <div class="modal-body" v-if="showModalContent">
            <div class="table-responsive">
              <table
                ...
                v-if="activeOrder.order.reminders.length"
              >
                <thead>
                  <tr>
                    <th>S/N</th>
                ...
                <tbody>
                  <tr
                    v-for="(reminder, index) in activeOrder.order.reminders"
                    :key="index"
                  >
                    <th>{{ index + 1 }}</th>
    ...
    <nav
      v-if="mode === 'renewal' && responseData.next_page_url"
      class="col d-flex justify-content-end align-items-center pr-0"
    >
      <ul class="pagination pagination-lg mb-0">
        <!---->
        <li
          :class="{ disabled: !responseData.first_page_url }"
          class="page-item"
        >
          <a href="javascript:" @click="prev(1)" class="page-link">First</a>
        </li>
        <li
          :class="{ disabled: !responseData.prev_page_url }"
          class="page-item"
        >
        ...
        <li class="page-item">
          <span class="page-link"
            >Current Page: {{ responseData.current_page }}</span
          >
        </li>
        <!---->
        <li
          :class="{ disabled: !responseData.next_page_url }"
          class="page-item"
        >
          <a href="javascript:" @click="next()" class="page-link">Next</a>
        </li>
        <li
          :class="{ disabled: !responseData.last_page_url }"
          class="page-item"
        >
          <a
            href="javascript:"
            @click="next(responseData.last_page)"
            class="page-link"
            >Last</a
          >
        </li>
...
<script>
  ...
  let url = (to) =>
    `/api/reminder/create?list=${
      to.query.list +
      (to.query.filterWithBranch
        ? `&filterWithBranch=` + to.query.filterWithBranch
        : "")
    }`;
  ...
  export default {
    ...
    methods: {
      ...
      async prepareForm({ orders }) {
        ...
        this.orders =
          await this.convertRequestOrdersToOrderClassInstance(orders);
      ...
      fetchList(list) {
        this.$LIPS(true);
        let filterWithBranch = !(
          this.auth("DVALead") ||
          this.auth("FSLLead") ||
          this.auth("CAGAccess")
        );
        get(url({ query: { list, filterWithBranch } }))
          .then(({ data }) => {
            if (list === 8)
              data.orders = data.orders.map((promiseCall) => promiseCall.order);
            this.prepareForm(data);
          })
          .catch(() => {})
          .finally(() => {
      ...
      sendSMSReminders(selectedOrders) {
        ...
        selectedOrders.forEach((order, index) => {
          let sms = new Message(
            order.nextSMSReminder,
            order.reminder.contacts,
            false,
            order.dvaId
          );
          sms.send((r) => {
            ...
            if (index + 1 === selectedOrders.length)
              this.logSentMessages(selectedOrders, messages);
          });
        });
      },
        ...
        try {
          const status =
            this.list === "Current"
              ...
              : this.list
              ? this.status.find(
                  (option) => option.status === this.list.toLowerCase()
                ).id
              : "";
          ...
          const url1 = `${base}${
            adminAccess ? queryParam(admin) : queryParam(nonAdmin)
          }`;
          const url0 = `${base}/status/${status}${
            adminAccess ? queryParam(admin) : queryParam(nonAdmin)
          }`;
      ...
      popIt(param) {
        this.list === "Current"
          ? this.postRenewal(param)
          : this.updateRenewal(param);
      },
      ...
      async updateRenewal(param) {
        ...
        try {
          await put(
            `/api/renewal-list/${param.data.renewalId}`,
            param.data
          ).then(() => this.postFeedback(param.feedback));
          this.orders = this.orders.filter(
            (p) => p.order.id != param.data.order_id
          );
        } catch (err) {
          this.$displayErrorMessage(err);
          this.$LIPS(false);
      ...
      async postRenewal(param) {
        this.$LIPS(true);
        try {
          await post(`/api/renewal-list`, param.data).then(() =>
            this.postFeedback(param.feedback)
          );
          this.orders = this.orders.filter(
            (p) => p.order.id != param.data.order_id
          );
    ...
    mounted() {
      this.mode === "renewal"
        ? this.fetchRenewalList(this.list)
        : this.mode != "normal-list"
        ? this.fetchList(this.list)
