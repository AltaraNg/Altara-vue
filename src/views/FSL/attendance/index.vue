    ...
    <div class="pt-md-3 pt-2 attendance attendance-view" id="index">
        ...
        <div class="mb-5 row align-items-center">
          <div class="col-12 title-con">
            ...
            <div class="row justify-content-end">
              ...
              <a @click="toggleGuide" class="text-link mt-3" href="javascript:">
                view table guide!</a
              >
            </div>
          </div>
      ...
      <div class="attendance-body" id="table-guide">
        <div class="pt-5 row bg-white shadow-sm card-radius">
          <div>
            ...
            <span
              >In before/at 9:00am <br />
              Out after/at 6:00pm</span
            >
          </div>
      ...
      <div class="mt-5 row attendance-head">
        <div
          ...
          v-for="capt in ['Branch', 'Month', 'Year']"
          :key="capt"
        >
          <div class="row">
            <div class="light-heading">
              <span class="d-none d-sm-inline">Select</span> {{ capt }}
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="mt-2 mt-lg-3 row attendance-head">
        <div
          class="col-4 col-sm-3"
          v-for="caption in ['branch', 'month', 'year']"
          :key="caption"
        >
          <div class="row">
            <select
            ...
            <small class="text-muted" v-if="errors.first(caption)">{{
        ...
        <div class="col-12 col-sm-3">
          <div class="row d-flex justify-content-end">
            <button
              @click="fetch()"
              class="btn btn-primary bg-default mt-0 myBtn"
            >
              View Attendance
            </button>
          </div>
      ...
      <div class="attendance-body">
                ...
                <thead>
                  <tr>
                    <th
                      ...
                      :key="col.month"
                    >
                      <span>{{ `${col.month} ${col.date}` }}</span
                      ><br /><span class="light-heading day">
                        {{ `${col.dayString}` }}
                      </span>
                    </th>
                  </tr>
        ...
        <div class="row attendance-item p-5 my-5" v-else>
          <span class="no-attendance"
            >Kindly Select Branch, Month and, Year to get started!</span
          >
        </div>
      </div>
                ...
                <div class="px-2" v-if="currAttendance">
                  <div class="px-4">
                    <div class="row">
                      ...
                      <div class="col">
                        {{ currAttendance.is_present ? "Present" : "Absent" }}
                      </div>
                    </div>
                    <div class="row">
                      <span><strong>Arrival Time : </strong></span>
                      <div class="col">
                        {{ $timeConvert(currAttendance.arrival_time) }}
                      </div>
                    </div>
                    <div class="row">
                      <span><strong>Departure time : </strong></span>
                      <div class="col">
                        {{ $timeConvert(currAttendance.departure_time) }}
                      </div>
                    </div>
                    <div class="row">
...
<script>
  ...
  const apiLink = (query) =>
    `/attendance?branch=${query.branch}&month=${query.month}&year=${query.year}`;
  export default {
    data() {
      return {
        columns: {},
    ...
    created() {
      ...
      Vue.set(
        this.$data.query,
        "month",
        month >= 10 || month.length === 2 ? month : "0" + month
      );
    ...
    methods: {
      ...
      isPresent(userAtt, day, bool = false) {
        ...
        if (c.length > 0)
          data = bool ? c[0][bool] : c[0].is_present ? "P" : "A";
        return data;
      },

      earlyOrLate(userAtt, day) {
        let data = [];
        let attendance = userAtt.attendances.filter(
          (att) => att.date === day.fullDate
        );
        if (attendance.length) {
          data[0] =
            attendance[0].arrival_time > "09:00" ? "arrLate" : "arrEarly";
          if (attendance[0].departure_time) {
            data[1] =
              attendance[0].departure_time < "18:00" ? "leftEarly" : "leftLate";
    ...
    computed: {
      ...
      title() {
        let att = "Attendance";
        if (this.completeQry) {
          let qryMonth = this.$route.query.month,
            qryYear = this.$route.query.year;
          const month = qryMonth
            ? " - " + this.$store.getters.getMonths[parseInt(qryMonth) - 1].name
            : "";
