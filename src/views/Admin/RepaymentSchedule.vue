    ...
    <div>
        ...
        <div class="center my-2 flex" style="gap: 10px">
            ...
            <select class="custom-select flex-1" v-validate="'required'" v-model="branch">
                ...
                <option v-for="option in Branches" :value="option.id" :key="option.id">{{ option.name }}</option>
            </select>
        ...
        <div class="px-4 flex w-full" style="gap: 40px; margin-top: 30px">
            <div class="card hover-zoom hover-shadow bg-white" v-for="(data, index) in StatData" :key="index">
                <div>
                    <p>{{ data.title }}</p>
                    <p style="color: #04103b; font-weight: 700; font-size: 25px; line-height: 0.3">{{ $formatCurrency(data.amount) || data.stat }}</p>
                    <div class="pt-4 flex" style="align-items: center">
                        <component :is="data.percentIcon" />
                        <div class="pl-1 font-weight-bold" :class="data.title == 'Total  Repayment Paid' ? 'text-success' : 'text-danger'">
        ...
        <div v-if="pageParams && orders.length > 0">
            <base-pagination :page-param="pageParams" @fetchData="getReport"> </base-pagination>
        </div>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        prepareList(response) {
            ...
            let { current_page, first_page_url, from, last_page, last_page_url, data, per_page, next_page_url, to, total, prev_page_url } = response;
