<template>
  <transition name="fade">
    <div id="reminder" class="my-4 container-fluid">
      <div
        class="d-flex gap-3 mt-5 justify-content-center"
      ...
      <div v-if="canGenerateList">
        <button class="bg-default p-2" @click="generateList">
          Generate List
        </button>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      async exportCsv() {
        ...
        this.$LIPS(true);
        try {
          const response = await get(this.apiUrl.exportCsv + queryParam(param));
          let fileURL = window.URL.createObjectURL(new Blob([response.data]));
          let fileLink = document.createElement("a");
          fileLink.href = fileURL;
          fileLink.setAttribute("download", "collections.csv");
      ...
      prepareList(response) {
        ...
        // eslint-disabled-next-line no-unsafe-optional-chaining
        let {
          current_page,
          first_page_url,
          from,
