<template>
  <div class="mb-3 row attendance-item">
    ...
    >
      ...
      <span
        v-else-if="
          order.reminder.canBeSelected &&
          ['collection', 'recovery', 'call', 'external-recovery'].includes(mode)
        "
        class="user mx-auto bg-pending text-white"
        @click="logReminder"
      >
        <i class="fas fa-hourglass-start"></i>
      </span>
      <div
        v-else-if="order.reminder.canBeSelected && mode === 'sms'"
        class="d-flex align-items-center"
      >
        <input
          class="form-check-input my-0 mx-4 float-left position-relative"
          type="checkbox"
          v-model="order.isSelected"
          @click="toggleSelect"
        />
      </div>
      <span class="user mx-auto sent-reminder" v-else-if="mode != 'renewal'"
    ...
    <div
      class="col-12 col-xs-2 col-md col-lg user-name d-flex align-items-center justify-content-center"
    >
      {{ $vnode.key }}
    </div>
    ...
    <div
      ...
      @click="$emit('display', order, 'purchase_order')"
      data-hoverable="true"
    >
    ...
    <div
      ...
      @click="$emit('display', order, 'customer_info')"
      data-hoverable="true"
    >
      ID: {{ order.customer.id }} -
      {{ order.customer.employment_status | capitalize }}
    </div>

    <div
      class="col-12 col-xs-2 col-md col-lg d-flex align-items-center justify-content-center"
      @click="$emit('display', order, 'repayment')"
      data-hoverable="true"
    >
    ...
    <div
      class="col-12 col-xs-2 col-md col-lg d-flex align-items-center justify-content-center"
      v-if="mode != 'renewal'"
      @click="$emit('display', order, 'reminder_history')"
      data-hoverable="true"
    >
    ...
    <div
      class="col-12 col-xs-2 col-md col-lg d-flex align-items-center attendance-create"
    ...
    >
      <textarea
        class="form-control"
        rows="1"
        v-model="order.reminder.feedback"
    ...
    >
      <input
        class="form-control"
        type="date"
        v-model="order.promiseCall.date"
        :disabled="!order.reminder.canBeSelected"
      />
    </div>
    ...
    <div
      class="col-12 col-xs-2 col-md col-lg d-flex align-items-center justify-content-center"
      v-if="mode === 'renewal' && tab === 'Successful'"
      @click="$emit('display', order, 'reminder_history')"
      data-hoverable="true"
    >
      {{
        order.order.reminders.length === 1
          ? "1 History"
          : order.order.reminders.length > 1
          ? order.order.reminders.length + " Histories"
          : "No History"
      }}
    </div>
    ...
    <div
      ...
      v-if="mode === 'renewal' && tab != 'Successful'"
    >
      <textarea v-model="feedback" class="form-control" rows="1"> </textarea>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      objCollector() {
        if (
          !this.status ||
          (!this.sChecker(this.status) && !this.dateTime) ||
          !this.feedback
        ) {
        ...
        if (
          Date.parse(Cdate) - Date.parse(today) < 1 ||
          Date.parse(Cdate) > nWeek
        ) {
          return this.errHandler("Pease enter a valid date.");
        }
        const renew = this.order.order.renewal
          ? this.order.order.renewal.id
          : "";
        const data0 = {
      ...
      sChecker(param) {
        return !param
          ? true
          : this.options.find((x) => x.id === param).status != "callback";
      },
...
<style scoped>
  ...
  .Callback {
    background-color: rgba(163, 163, 104, 0.09);
    color: #ffa500;
  }
  .Unreachable {
    background-color: rgba(163, 0, 104, 0.09);
    color: #e30000;
  }
  .option2 {
