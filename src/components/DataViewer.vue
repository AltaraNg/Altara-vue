        ...
        <div class="card">
          <div class="px-5 py-4">
            <div class="px-3 clearfix">
              <h5 class="h5-custom float-left m-0">
                {{ $route.meta.appModel | capitalize }} Management
              </h5>
              <router-link
                :to="`${$route.meta.new}/create`"
          ...
          <div>
            ...
            <div class="card-body p-4 p-md-5">
              <div class="clearfix w-100">
                <div
                  class="form-group col-md-2 col-sm-6 px-md-3 px-1 float-left"
                >
                ...
                <div
                  class="form-group col-md-6 col-sm-6 px-md-3 px-1 float-left"
                >
                ...
                <div
                  class="form-group col-md-2 col-sm-6 px-md-3 px-1 pt-md-2 pt-0 float-left"
                >
                  <button
                    @click="fetchIndexData()"
                    class="btn btn-block bg-default mb-0 mt-3 mt-md-4"
                  >
                    Filter
                  </button>
                </div>
                  ...
                  <tbody>
                    <tr v-for="model in model.data" :key="model">
                      <td v-for="(value, key) in model" :key="key">
                        <span v-if="!['verification'].includes(key)">{{
                          value
                        }}</span>
                      ...
                      <td v-if="isModel('user')">
                        ...
                        <button
                          :class="{
                            'btn-success': model.portal_access,
                      ...
                      >
                        <button
                          @click="
                            $router.push(`${$route.meta.new}/${model.id}/edit`)
                          "
                    ...
                    <div class="modal-body">
                      <div class="form-group col-12 float-left mt-0 mb-2">
                        <span class="mb-2 w-100 float-left pl-1 text-center">
                          Please Verify you selected the right access before
                          clicking <br />
                          <strong>Save Changes </strong>!
                        </span>
                        <div
                          class="radio p-0 col-6 float-left text-center"
                    ...
                    <div class="modal-body">
                      <div class="form-group col-12 float-left mt-2 mb-4">
                        <span
                          ...
                          <strong>sms</strong> on the telephone He/She
                          <strong>used for registration</strong> on this portal.
                          <br /><br />Please Kindly verify that the phone to
                          receive the new password is on and active!
                        </span>
                        <br /><br />
                        <u
                          ><em
                            >click the continue and reset password to finish
                            this task!</em
                          ></u
                        >
                      </div>
  ...
  export default {
    ...
    async mounted() {
      ...
      try {
        this.$LIPS(true);
        $(".modal").modal("hide");
        get(`${this.$route.meta.source}` + queryParams(this.query)).then(
          (res) => {
            ...
            if (data.length) {
              data.forEach((curr) => {
                if (data[0].state_id)
                  curr.state_id = this.$store.getters?.getStates?.find(
                    (obj) => obj.id === curr.state_id
                  )?.name;
                if (data[0].branch_id)
                  curr.branch_id = this.$store.getters?.getBranches?.find(
                    (obj) => obj.id === curr.branch_id
                  )?.name;
                if (this.isModel("customer"))
                  curr.verification = this.$getCustomerApprovalStatus(
                    curr.verification
                  );
              });
            }
            this.pageCount = Math.ceil(
              res.data.model.total / res.data.model.per_page
            );
        ...
        if (this.$network()) {
          ...
          get(`/api/reset-password/${this.form.id}`).then(({ data }) => {
            let { password: psw } = data, //extract password from the data received
              ...
              body =
                `Password reset successful! if your did not request for a new password kindly` +
                ` report back immediately, your staff ID is ${id}, new password: ${psw}`;
            log("resetUserPassword", id);
            new Message(body, tel, false).send(
              (r) => r.status === 200 && this.done()
            );
        ...
        if (this.$network()) {
          ...
          byMethod("PUT", `/api/user/${id}`, this.form)
            ...
            .catch(({ response: r }) => {
              let { message } = r.data;
              Flash.setError(
                message ? message : "Error updating status. Try again later!",
                10000
              );
            })
            .finally(() => this.done());
