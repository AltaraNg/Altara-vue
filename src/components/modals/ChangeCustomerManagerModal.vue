    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header py-2">
          <h6 class="modal-title py-2">Change Manager for Customer</h6>
          <a
            aria-label="Close"
            class="close py-1"
            data-dismiss="modal"
            ...
            <div class="clearfix">
              <div class="form-group">
                ...
                <input
                  class="form-control"
                  :value="
                    $getCustomerFullName(customer) + ' - ID: ' + customer.id
                  "
                  type="text"
                  disabled
                />
        ...
        <div class="modal-footer d-flex justify-content-between">
          <button class="btn status approved" @click="changeCustomerManager">
            save changes
          </button>
        ...
        post("/api/update_customer_manager", data)
          .then((response) => {
            if (response.status === 201) {
              log(
                `CustomerAssignedNewDsa`,
                "CustomerId: " + this.customer.id + " newDsa: " + this.DSAId
              );
              Flash.setSuccess(
                "A new DSA has been assigned to the customer.",
                5000
              );
