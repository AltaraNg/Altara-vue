          ...
          <div class="my-4 clearfix p-5 row bg-white shadow-sm card-radius">
            <div class="form-group col-md-3 col-sm-6 px-md-3 px-1 float-left">
              ...
              >
                ...
                <option
                  :value="slug"
                  v-for="{ slug, name } in types"
                  :key="name"
                >
                  {{ name | capitalize }}
                </option>
              </select>
              <small v-if="errors.first('f1.report_type')">{{
                errors.first("f1.report_type")
              }}</small>
            </div>
            ...
            <div class="form-group col-md-3 col-sm-6 px-md-3 px-1 float-left">
              <label>Branch</label>
              <select
                :disabled="!$store.getters.auth('DSALead')"
                class="custom-select w-100"
                data-vv-as="office branch"
                data-vv-validate-on="blur"
                name="branch_id"
                v-model="report.branch.id"
                v-validate="'required'"
              >
                <option value="">select branch</option>
                <option
                  :value="branch.id"
                  v-for="branch in $store.getters.auth('DSALead')
              ...
              <small v-if="errors.first('f1.branch_id')">{{
                errors.first("f1.branch_id")
              }}</small>
            </div>
            ...
            <div class="form-group col-md-3 col-sm-6 px-md-3 px-1 float-left">
              ...
              />
              <small v-if="errors.first('f1.date_from')">{{
                errors.first("f1.date_from")
              }}</small>
            </div>
            ...
            <div class="form-group col-md-3 col-sm-6 px-md-3 px-1 float-left">
              ...
              <input
                ...
                v-validate="'required|date_format:MM/DD/YYYY'"
              />
              <small v-if="errors.first('f1.date_to')">{{
                errors.first("f1.date_to")
              }}</small>
            </div>
          </div>
          ...
          <div class="my-4 clearfix p-5 row bg-white shadow-sm card-radius">
            ...
            <div class="form-group col-md-2 col-sm-6 px-md-3 px-1 float-left">
              ...
              <input
                ...
                v-validate="'required|date_format:MM/DD/YYYY'"
              />
              <small v-if="errors.first('f2.date')">{{
                errors.first("f2.date")
              }}</small>
            </div>
            ...
            <div class="form-group col-md-2 col-sm-6 px-md-3 px-1 float-left">
              ...
              <input
                ...
                type="number"
                v-model="dailyReport.number_on_portal"
                v-validate="'required|integer|min:0'"
              />
              <small v-if="errors.first('f2.number_on_portal')"
                >{{ errors.first("f2.number_on_portal") }}
              </small>
            </div>
            ...
            <div class="form-group col-md-2 col-sm-6 px-md-3 px-1 float-left">
              ...
              <input
                ...
                type="number"
                v-model="dailyReport.number_to_captain"
                v-validate="'required|integer|min:0'"
              />
            ...
            <div class="form-group col-md-3 col-sm-6 px-md-3 px-1 float-left">
              ...
              ></textarea>
              <small v-if="errors.first('f2.remark')">{{
                errors.first("f2.remark")
              }}</small>
            </div>
          </div>
          <div class="mb-5 px-0 row align-items-center">
...
<script>
  ...
  export default {
    ...
    beforeCreate() {
      ...
      get(`/api/user/getBranchUsers`).then(
        ({ data }) => (this.users = data.DSAs)
      );
    ...
    methods: {
      ...
      generateReport() {
        this.$validator.validateAll("f1").then((result) => {
          if (result) {
            if (this.$network()) {
              let { id, name } = this.$store.state.branches.find(
                ({ id }) => id === this.report.branch.id
              );
              ...
              postD("/api/report", this.report).then(({ data }) => {
                ...
                link.setAttribute(
                  "download",
                  `${this.report.type}_for_${name}.xlsx`
                );
          ...
          if (result) {
            ...
            if (this.$network()) {
              ...
              post(`/api/dsa_daily_registration`, this.dailyReport)
                ...
                .catch(() =>
                  Flash.setError("Error logging report please try again later!")
                );
            } else this.$networkErr();
          } else this.$networkErr("form");
        });
