  ...
  <div>
    ...
    <div class="center my-2 flex" style="gap: 10px">
      ...
      >
        ...
        <option
          v-for="option in availableRepaymentPlan"
          :value="option.id"
          :key="option.id"
        >
          {{ option.name }}
      ...
      >
        ...
        <option v-for="option in Branches" :value="option.id" :key="option.id">
          {{ option.name }}
        </option>
      </select>
    ...
    <div class="px-4 flex w-full" style="gap: 40px; margin-top: 30px">
      <div
        class="card hover-zoom hover-shadow bg-white"
      ...
      >
        <div>
          ...
          <p
            ...
            "
          >
            {{ $formatCurrency(data.amount) || data.stat }}
          </p>
          <div class="pt-4 flex" style="align-items: center">
            ...
            <div
              ...
              :class="
                data.title == 'Total  Repayment Paid'
                  ? 'text-success'
                  : 'text-danger'
              "
            >
              {{ data.percent != "NaN%" ? data.percent : "0%" }}
            </div>
          </div>
        </div>
    ...
    <div
      class="mx-4"
      style="margin-top: 30px; margin-bottom: 40px; padding-left: 20px"
    >
    ...
    methods: {
      fetchData() {
        ...
        this.pageParams.per_page =
          this.pageParams.per_page || this.$route.query.per_page;
          ...
          datasets: [
            {
              ...
              data: Object.values(this.reports.groupByMonth).map(
                (item) => item.expected_repayment
              ),
            },
            {
        ...
        try {
          ...
          this.StatData = [
            ...
            },
            {
              ...
              percent:
                this.getPercent(
                  this.reports.actual_repayment,
                  this.reports.expected_repayment
                ) + "%",
              percentIcon: arrowUp,
              icon: tracking,
            },
            {
              ...
              percent:
                this.getPercent(
                  this.reports.defaultingOrders,
                  this.reports.total_orders
                ) + "%",
              percentIcon: arrowDown,
              icon: users,
            },
          ...
          this.MonthlyStat = Object.values(this.reports.meta.groupByMonth);
        } catch (err) {
          if (err.response) {
      ...
      async getRepaymentDuration() {
        try {
          const fetchRepaymentDuration = await get(
            this.apiUrls.repaymentDurations
          );
