<template>
    <div class="my-3 py-4" v-if="reports !== null">
        ...
        <div class="center my-2 flex px-4" style="gap: 10px">
            <date-picker class="flex-1" v-model="fromDate" valueType="format" placeholder="From Date"></date-picker>

            <date-picker class="flex-1" v-model="toDate" valueType="format" placeholder="To Date"></date-picker>
            ...
            <select class="custom-select flex-1" v-model="branch" v-validate="'required'">
                <option value="">Showroom</option>
                <option :value="branch.id" v-for="branch in getBranches" :key="branch.id">{{ branch.name }}</option>
            </select>
        </div>

        <div class="flex mt-3 mb-5 px-4">
            <button class="bg-default rounded ml-auto py-2 px-4" @click="filterByDate">
                <span class="h5" style="width: 5rem"> Filter </span>
            </button>
        </div>
        ...
        <div class="row px-4 h-auto" v-if="reports !== null">
            <stat-card :stat="totalNumberOfSales" class="col mx-4 w-50 font-weight-bold" :label="'Total Number of Sales'">
        ...
        <div class="my-3 d-flex px-4 w-100">
            ...
            <div class="card col p-5" v-if="reports !== null">
                <pie-chart :chart-data="pieData" :options="pieData.options" v-if="loaded" class=""></pie-chart>
                <div class="text-left">
                    <h4>Overview</h4>
                    <ul class=".list-unstyled">
                        <li>
                            <span class="font-weight-bold" style="color: #e2e612">Active Sales</span>:
                            <span
                                >anyone who has not completed payment but has made a payment in the last 60 days(2 months) (from today's date),
                                REGARDLESS of if the payment cycle is over.</span
                            >
                        </li>
                        <li>
                            <span class="font-weight-bold" style="color: #e6091c">Inactive Sales</span>:
                            <span> anyone who has not completed payment but has not made any payment in the last 60 days</span>
                        </li>
                        <li>
                            <span class="font-weight-bold" style="color: #2ad413">Completed Sales</span>:
                            <span>anyone who has completed payment irregardless of if they completed it on time or late. </span>
                        </li>
                        <li v-if="checker">
                            <span class="font-weight-bold" style="color: #aaaaaa">No Orders </span>: <span>No orders for that time frame </span>
                        </li>
                    </ul>
                </div>
            </div>
            ...
            <div class="card col ml-5 p-5" v-if="reports">
                <polar-chart :chart-data="polarData" :options="polarData.options" v-if="loaded" class=""></polar-chart>
...
export default {
    ...
    data() {
        return {
            ...
            productPieData: {
                labels: [],
                dataSet: [],
                bgColor: ["#ff0000", "#00aeae", "#7580ce", "#007900", "#39f159", "#989000", "#20062b", "#dc5355", "#5526c4", "#ff7f00"],
                options: {
                    responsive: true,
    ...
    methods: {
                            ...
                            label: function (tooltipItem, data) {
                                ...
                                var percent =
                                    (data.datasets[0].data[indice] /
                                        (data.datasets[0].data[0] + data.datasets[0].data[1] + data.datasets[0].data[2])) *
                                    100;
                                return (
                                    data.labels[indice] + ": " + data.datasets[0].data[indice] + " (" + Number.parseFloat(percent).toFixed(2) + "%)"
                                );
                            },
                        },
                    },
        ...
        getPolarData() {
            ...
            return [orderStatus["1_30"], orderStatus["31_45"], orderStatus["45_above"]];
        },
        async getReport() {
            this.$LIPS(true);
        ...
        getSums(dataArray) {
            ...
            for (let index = 0; index < dataArray.length; index++) {
                ...
                this.sums.totalRevenue += parseFloat(dataArray[index].total_potential_revenue_sold_per_showroom.replace(/,/g, ""));
                this.sums.totalAltPay += dataArray[index].no_of_altara_pay;
                this.sums.totalAltCash += dataArray[index].no_of_altara_cash;
                this.sums.totalAvePerProd += parseFloat(dataArray[index].avg_price_of_prod_per_showroom.replace(/,/g, ""));
                this.sums.totalPercent += Number(dataArray[index].percentage_of_total_revenues);
                this.sums.totalForecast += Number(dataArray[index].forecast);
                this.sums.averageDownPayment += Number(dataArray[index].percentage_downpayment);
            }
        },
        ...
        getPieData() {
            const orderStatus = this.reports?.meta?.stats?.ordersStatusCount;
            // this.checker = orderStatus.active === 0 && orderStatus.inactive === 0 && orderStatus.complete === 0;
            return this.checker === true ? [1] : [orderStatus.active, orderStatus.inactive, orderStatus.complete];
        },
        ...
        getBarchartColors() {
            ...
            let newColors = [];
            let colors = ["#808080", "#808080", "#808080", "#808080", "#808080", "#808080", "#808080"];
