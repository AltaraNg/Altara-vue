  ...
  <div class="my-3 py-4" v-if="reports !== null">
    ...
    <div style="display: flex" class="my-3 px-4">
      <div class="border">
        ...
        <div v-for="(item, index) in TotalSalesBySalesCategory" :key="index">
          <p class="progress-text pt-3 text-capitalize">
            {{ item.sales_category_name }}
          </p>
          <div
            class="progress-bar d-flex justify-content-between align-items-center"
          >
            <div
              class="progress d-flex justify-content-between align-items-center"
              :style="{ width: item.progress + '%' }"
            ></div>
            <p class="text text-center pl-3">
              {{ item.total_sales }} ({{ item.progress }}%)
            </p>
          </div>
        </div>
        <div class="d-flex justify-content-between text">
    ...
    <div style="display: flex" class="my-3 px-4">
      <div class="border">
        ...
        <div
          v-for="(item, index) in totalSalesByDownPaymentsAndRepaymentDuration"
          :key="index"
        >
  ...
  export default {
    ...
    methods: {
      ...
      addPercentageToSalesCategory() {
        this.SalesBySalesCategoryTotal = this.TotalSalesBySalesCategory.reduce(
          (sum, item) => sum + item["total_sales"],
          0
        );
        this.TotalSalesBySalesCategory = this.TotalSalesBySalesCategory.map(
          (item) => {
            const percentage =
              (item["total_sales"] / this.SalesBySalesCategoryTotal) * 100;
            return { ...item, progress: parseFloat(percentage.toFixed(2)) };
          }
        );
      },
      addPercentageToDownpaymentDuration() {
        this.totalSalesByDownPaymentsAndRepaymentDuration =
          this.totalSalesByDownPaymentsAndRepaymentDuration.sort((a, b) => {
            ...
            const monthComparison =
              monthOrder[partsA.months] - monthOrder[partsB.months];
        ...
        this.TotalByDP_RDTotal =
          this.totalSalesByDownPaymentsAndRepaymentDuration.reduce(
            (sum, item) => sum + item["total_sales"],
            0
          );
        this.totalSalesByDownPaymentsAndRepaymentDuration =
          this.totalSalesByDownPaymentsAndRepaymentDuration.map((item) => {
            const percentage =
              (item["total_sales"] / this.SalesBySalesCategoryTotal) * 100;
            return { ...item, progress: parseFloat(percentage.toFixed(2)) };
          });
      ...
      getPieChartData() {
        const valid =
          JSON.stringify(this.getPieData()) == JSON.stringify([0, 0]);
        this.pieData = valid
          ? this.ZeroPieData
          : {
              labels: ["Altara Cash", "Altara Pay"],
      ...
      async getReport() {
        ...
        this.billboardOrders
          ? (this.query.billboardOrders = "true")
          : delete this.query.billboardOrders;
        try {
          ...
          this.TotalSalesBySalesCategory =
            this.reports.meta.TotalSalesBySalesCategory;
          this.totalSalesByDownPaymentsAndRepaymentDuration =
            this.reports.meta.totalSalesByDownPaymentsAndRepaymentDuration;
