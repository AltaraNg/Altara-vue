  ...
  <transition name="fade">
    <div class="pt-md-3 pt-2 attendance-view" id="index">
        ...
        <div :class="`col-${col}`" v-for="{ name, col } in columns" :key="col">
          <div class="row">
            <div class="light-heading">
              <span class="d-none d-sm-inline"></span> {{ name }}
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="mt-1 attendance-body">
        <div v-if="show">
          <div
            class="mb-3 px-0 row align-items-center attendance-item"
            v-for="(caution, index) in cautions.data"
            :key="index"
            @click="displayInfo(caution)"
            data-toggle="tooltip"
            data-placement="top"
          ...
          >
            ...
            <div class="col-12 col-xs-2 col-md-2 col-lg-2 sm-hide">
              <div class="row">
                <span>{{ caution.reason_min }}</span>
              </div>
            </div>
            <div
              class="col-6 col-xs-2 col-md-2 col-lg-2 pr-3 py-4 py-sm-0 px-sm-0 px-lg-4 sm-hide"
            >
              <div class="row">
                <span>{{ caution.penalty_min }}</span>
              </div>
            </div>
            ...
            <div class="clearfix w-100 mt-4">
              <div class="float-left">
                <strong class="light-heading float-left">
                  Displaying: {{ cautions.from }} - {{ cautions.to }} of
                  {{ cautions.total }}
                </strong>
              </div>
              <nav aria-label="Page navigation example" class="float-right">
                <ul class="pagination pagination-lg float-left">
                  <li
                    :class="!cautions.first_page_url ? 'disabled' : ''"
                    class="page-item"
                  >
                  ...
                  <li
                    :class="!cautions.prev_page_url ? 'disabled' : ''"
                    class="page-item"
                  >
                    <router-link
                      :to="{ query: { page: cautions.current_page - 1 } }"
                      class="page-link"
                    >
                      Previous
                    </router-link>
                  ...
                  <li class="page-item">
                    <span class="page-link"
                      >Current Page: {{ cautions.current_page }}</span
                    >
                  </li>
                  <li
                    :class="!cautions.next_page_url ? 'disabled' : ''"
                    class="page-item"
                  >
                    <router-link
                      :to="{ query: { page: cautions.current_page + 1 } }"
                  ...
                  <li
                    :class="!cautions.last_page_url ? 'disabled' : ''"
                    class="page-item"
                  >
                    <router-link
                      :to="{ query: { page: cautions.last_page } }"
                      class="page-link"
                    >
                      Last
                    </router-link>
