    ...
    <div id="reminder" class="attendance">
      ...
      <div class="mt-5 mb-3 attendance-head">
        ...
        <div class="row px-4 pt-3 pb-4 text-center">
          ...
          <div
            class="col light-heading"
            v-if="evalMode === 'collection' || evalMode === 'recovery'"
          >
            Visited?
          </div>
          <div
          ...
          <div class="col light-heading" v-if="evalMode === 'call'">
            Promise Date
          </div>
          <div class="col light-heading" v-if="evalMode === 'recovery'">
            Action
          </div>
        </div>
      </div>
      <div class="tab-content mt-1 attendance-body">
        ...
        >
          <div class="col d-flex align-items-center" style="max-width: 120px">
            <span
              class="user mx-auto"
              :class="{ isProcessed: order.isProcessed }"
              v-if="
                evalMode === 'call' ||
                evalMode === 'collection' ||
                evalMode === 'recovery'
          ...
          <div
            class="col d-flex align-items-center justify-content-center overflow-auto"
            ...
            <span>
              <span class="radio w-50 pr-3 mb-0 float-left">
                <input
                  type="radio"
                  :value="true"
                  v-model="order.is_visited"
                  :id="`present${index}`"
                  v-if="!order.isProcessed"
                  v-validate="'required'"
                  :name="`isPresent${index}`"
                  :disabled="order.isProcessed"
                  class="form-check-input"
                />
                <label :for="`present${index}`" v-if="!order.isProcessed"
                  >yes</label
                >
                <input
                  type="radio"
                  :value="true"
                  v-model="order.visited"
                  :id="`present${index}`"
                  v-if="order.isProcessed"
                  v-validate="'required'"
                  :name="`isPresent${index}`"
              ...
              <span class="radio w-50 pl-3 mb-0 float-left">
                <input
                  ...
                  class="form-check-input"
                />
                <label :for="`absent${index}`" v-if="!order.isProcessed"
                  >no</label
                >
                <input
                  type="radio"
          ...
          <div
            class="col d-flex align-items-center justify-content-center"
            v-if="evalMode === 'recovery'"
          >
            <button
              class="bg-default btn"
              @click="confirmModal(order)"
              :disabled="order.status === 'Repossessed'"
            >
              Repossess
            </button>
          </div>
                    ...
                    <tbody>
                      <tr v-for="(object, index) in modalItem" :key="index">
                        ...
                        <td>
                          {{
                            object.type ===
                            "App\\Notifications\\SmsReminderSent"
                              ? object.data.message
    ...
    methods: {
      ...
      save(order) {
        if (!order.isProcessed) {
          ...
          if (this.evalMode === "call") {
            if (
              order.feedback === undefined ||
              order.promise_date === undefined
            ) {
            ...
            data = {
              ...
              status: "called",
              promise_date:
                order.promise_date === true ? null : order.promise_date,
            };
          } else {
            data = {
              feedback: order.feedback,
          ...
          post(this.urlToFetchOrders, data)
            .then(({ data }) => {
      ...
      repossess(elem) {
        if (elem) {
          //send the request
          this.$LIPS(true);
          patch(`/api/new-order/${this.selectedOrder.id}/repossess`)
            .then(() => {
              this.fetchData();
              this.$swal({
                icon: "success",
    ...
    watch: {
      modeType: function (modeType) {
        ...
        if (
          ...
          modeType === "first_call" ||
          modeType === "second_call" ||
          modeType === "third_call"
        ) {
          this.evalMode = "call";
        } else if (
          modeType === "first_collection" ||
          modeType === "second_collection" ||
          modeType === "third_collection"
        ) {
          this.evalMode = "collection";
        } else if (
          modeType === "first_recovery" ||
          modeType === "second_recovery" ||
          modeType === "third_recovery" ||
          modeType === "recovery"
        ) {
