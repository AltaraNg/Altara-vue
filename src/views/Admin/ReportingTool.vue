<template>
    <div class="my-3 py-4" v-if="reports !== null">
        <div class="space-between mb-5 px-4">
            ...
            <button class="bg-default rounded py-2 px-4 h5" @click="exportReportCsv"><i class="fas fa-download mr-3"></i>Download</button>
        </div>
        ...
        <div class="center my-2 flex px-4" style="gap: 10px">
            ...
            <div class="col d-flex align-items-center" v-if="flag" style="font-size: 8.5px">
                <toggle-button
                    ...
                    :switchName="'billboardOrders'"
                    :key="'billboardOrders'"
                    :defaultState="billboardOrders"
                    :label="'Billboard Orders'"
                />
            </div>
        </div>
        ...
        <div class="row px-4" v-if="reports !== null">
            <stat-card :stat="reports.meta.total_no_sales" class="col mx-4 w-50" :label="'Total Number of Sales'">
                <template v-slot:svg> <Sales /> </template>
            </stat-card>
            <stat-card :stat="'₦' + reports.meta.revenue_per_sale" class="col mx-4 w-50" :icon="'fas fa-dolly-flatbed'" :label="'Revenue per Sale'">
                <template v-slot:svg> <Revenue /> </template>
            </stat-card>
            <stat-card :stat="'₦' + reports.meta.total_revenue" class="col mx-4 w-50" :label="'Total Revenue'" :icon="'fas fa-dolly-flatbed'">
        ...
        <div class="my-3 space-between px-4 w-100 text-center">
            <div class="card col mr-3 py-2" v-if="reports">
                <bar-chart :chart-data="barData" :options="barOption" v-if="loaded"></bar-chart>
            </div>

            <div class="card col ml-3 py-2" v-if="reports">
                <pie-chart :chart-data="pieData" :options="option" v-if="loaded" class=""></pie-chart>
            </div>
        </div>
            ...
            <div class="border">
                ...
                <div v-for="(item, index) in TotalSalesBySalesCategory" :key="index">
                    ...
                    <div class="progress-bar d-flex justify-content-between align-items-center">
                        <div class="progress d-flex justify-content-between align-items-center" :style="{ width: item.progress + '%' }"></div>
        ...
        <div class="my-4 mx-3 w-100 pt-3">
            ...
            <div class="ml-4 mr-5 mt-3 bg-white shadow">
                <table class="table table-responsive table-striped w-100">
                    <thead>
                        <tr>
                            <th v-for="(header, index) in tableHeaders" class="font-weight-bolder h5 text-center" :key="index">
                                {{ header }}
                            </th>
                        </tr>
                    </thead>
                    <tbody v-if="reports !== null">
                        <tr v-for="(branch, index) in branchesInfo" :key="index" class="text-center">
                            <td>{{ index + 1 }}</td>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row product-pie-card">
                        <div class="col-md-4">
                            <pie-chart :chart-data="productPieData" :options="productPieData.options" v-if="loaded" class=""></pie-chart>
                        </div>
                        <div class="col-md-8">
                            <ul class="list-disc pl-5">
                                <li
                                    ...
                                    :key="index"
                                >
                                    <span class="text-left text-black">{{ productPieData.labels[index] }}: </span><span class="">{{ item }}</span>
                                </li>
                            </ul>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <template v-if="reports">
                                <bar-chart :chart-data="OrderBarChart.data" :options="OrderBarChart.options" v-if="loaded"></bar-chart>
                            </template>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        getBranchLabel() {
            ...
            return branches.map((obj) => obj.branch_name).sort((a, b) => a.localeCompare(b));
        },
        ...
        getSalesPerBranch() {
            ...
            return branches.sort((a, b) => a.branch_name.localeCompare(b.branch_name)).map((obj) => obj.number_of_sales);
        },
